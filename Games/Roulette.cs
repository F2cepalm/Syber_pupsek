        ...
        static InlineKeyboardButton[][] keyboardBasic = new[]
{
        ...
        static internal async Task Start(Update update, CancellationToken cts)
        {
            ...
            {
                if (update.Message.From.Username != null || update.CallbackQuery.From.Username != "")
                {
                ...
                {
                    await bot.SendTextMessageAsync(update.Message.Chat.Id, "Вы не можете использовать данную команду без указания тега в вашем профиле.");
                }
            }
        }
        ...
        {
            if (upd.CallbackQuery.From.Username != null || upd.CallbackQuery.From.Username != "")
            {
            ...
            {
                await bot.SendTextMessageAsync(upd.Message.Chat.Id, "Вы не можете использовать данную команду без указания тега в вашем профиле.");
            }
        }
        ...
        {
            if(gameList.First(gm => gm.GameId == upd.CallbackQuery.Message.Chat.Id).Status == "Joining")
            {
