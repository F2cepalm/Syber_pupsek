...
namespace CyberPupsekBot.Functions
{
    ...
    {
        async public static void Send(ITelegramBotClient bot, Update update)
        {
            if(update.Message.Text.StartsWith("/help"))
            {
                string helpMessage = "Помощь по командам: \n" + 
                                     ...
                                     "Настройки: Бот будет работать по заданным настройкам до перезапуска, а потом вернет все к исходным" + "\n" + 
                                     "@AllowRahat , @KillRahat - разрешает и запрещает пользователю Рахат совершать действия в текущем чате";
